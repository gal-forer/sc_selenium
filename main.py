...
def get_records(first_name: str, last_name: str, from_date: str, thru_date: str, chrome_driver: str) -> str:
    try:
        ...
        wait.until(EC.visibility_of_element_located((By.XPATH, "//*[@id='criteria_full_name']")))
        # getting the search text boxes and populating them
        if first_name or last_name:
            driver.find_element(By.XPATH, "//*[@id='criteria_full_name']").send_keys(f'{first_name} {last_name}')
        # search.send_keys("ben smith")
        if from_date:
            driver.find_element(By.XPATH, "//*[@id='criteria_file_date_start']").send_keys(from_date)
        if thru_date:
            driver.find_element(By.XPATH, "//*[@id='criteria_file_date_end']").send_keys(thru_date)
        # Click search button
        ...
        while has_next_page:
            ...
            wait.until(EC.visibility_of_element_located((By.XPATH, "//*[@id='grid_inst_num']")))
...
@click.command()
@click.option('--first-name', default='john', help='Your first name')
@click.option('--last-name', default='', help='Your last name')
@click.option('--from-date', default='04/12/2024', help='From date')
@click.option('--thru-date', default='04/19/2024', help='Thru date')
